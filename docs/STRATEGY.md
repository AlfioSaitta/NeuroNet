...
## Stato Attuale
Jarvis è **100% locale** — nessuna dipendenza da provider cloud. L'unica eccezione pianificata è l'offloading GPU verso un Worker sulla VPN Tailscale (non ancora deployato).
...
| **Contesto lunghissimo** | Max 12K token (locale) | Gemini 1M+ token |
...
**Nuovi file:**
- `jarvis/external/providers.py` — Classe base astratta `BaseProvider` + `ProviderRouter` (già esistente, 361 righe)

**Modifiche a file esistenti:**
- `jarvis/core/config.py` — Variabili `GEMINI_API_KEY`, `GEMINI_MODEL`, `EXTERNAL_PROVIDER_STRATEGY`
- `jarvis/core/llm_engine.py` — Integrazione `ProviderRouter` nel flusso `generate_chat`
- `jarvis/agent/prompt.py` — Routing selettivo per web knowledge / classificazione
...
| **2** | Completare `jarvis/external/providers.py` con `BaseProvider` + `ProviderRouter` | Alta |
| **3** | Creare `jarvis/external/gemini_provider.py` con wrapper Google Generative AI | Alta |
