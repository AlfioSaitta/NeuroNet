...
from core.config import MODEL_ID, LLAMA_MODEL_PATH, LLAMA_EMBED_MODEL_PATH
...
def _model_entry(model_id: str, owned_by: str = "jarvis",
    ...
    # Try to get a meaningful timestamp from core.config
...
async def openai_models():
    ...
    try:
        from core.config import EXTERNAL_PROVIDERS
        for provider_id in EXTERNAL_PROVIDERS:
            models.append(_model_entry(provider_id, f"jarvis-{provider_id}"))
    except (ImportError, AttributeError):
        pass

    # Chat model variations from core.config (if a different model string is set)
    try:
        from core.config import LLAMA_MODEL_PATH as _lm_path
