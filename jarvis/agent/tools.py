...
async def execute_tool_call(tool_call, bot=None, chat_id=None, confirmation_mgr=None):
    ...
    """
    from agent.confirmation import PendingConfirmation, AutoProvider, ConfirmationManager
...
# Inject load_skill and skill_discover tools
TOOLS_SCHEMA.append({
    "type": "function",
    "function": {
        "name": "load_skill",
...
# Inject skill tools from agent.skills (Jarvis-native paths)
try:
    from agent.skills import get_skill_tools
...
async def refresh_mcp_tools_async() -> int:
    ...
    try:
        from api.mcp.client import get_mcp_manager
        manager = get_mcp_manager()
        mcp_tools = await manager.discover_all_tools()
        TOOLS_SCHEMA.extend(mcp_tools)
        count = len(mcp_tools)
        if count:
            logger.info(f"MCP: {count} tools injectati in TOOLS_SCHEMA")
    except ImportError:
        pass
