...
if TELEGRAM_ENABLED:
    ...
    from api.auth.user_manager import _set_telegram_cache_invalidator
    ...
    async def telegram_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == 'ls':
            from rag.engine import generate_telegram_ls_data
            ...
            try:
                from scheduler.tasks import load_tasks
                active_todos = len([t for t in load_tasks().values() if t.get('status') != 'done'])
                from scheduler.cron import load_jobs
            ...
            from memory.backup import export_memory_to_json
        ...
        # ── Istruisci Jarvis (agy flow) ──
            ...
            from tg_bot.userbot import load_user_config
            ...
            from tg_bot.userbot import active_clients, get_session_path, get_config_path
            ...
            from scheduler.tasks import remove_todo
            ...
            from scheduler.cron import remove_cron_job
    ...
    async def handle_telegram_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_text == "📁 Esplora Progetti":
            ...
            if "telegram_userbot_manager" not in globals() and "telegram_userbot_manager" not in sys.modules:
                try:
                    import tg_bot.userbot as telegram_userbot_manager
                except ImportError:
                    await msg.reply_text("❌ Funzionalità Userbot disabilitata dal server.")
                    return
                    
            from tg_bot.userbot import active_clients
        ...
        if session and session.get("userbot_state"):
            ...
            try:
                from tg_bot.userbot import request_otp, sign_in_otp
            except ImportError:
            ...
            if state_val == "awaiting_phone":
                ...
                from tg_bot.userbot import load_user_config, save_user_config
        ...
        if session and session.get("admin_state"):
            ...
            from core.config import ALLOWED_USERS, save_allowed_users
        ...
        if msg.document:
            ...
            if session and session.get("admin_state") == "awaiting_restore":
                ...
                from memory.backup import import_memory_from_json
        ...
        try:
            ...
            while iterations < max_iterations:
                ...
                if raw_content:
                    from agent.tags import strip_thinking_blocks
            ...
            from agent.tags import process_all_tags, TagContext, telegram_prepare_markdown
            from tg_bot.format import telegram_safe_format
            from core.config import MODEL_PROFILE as _mdl
