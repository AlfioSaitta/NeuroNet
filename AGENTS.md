...
| `agent/prompt.py` | ✅ **ESTESO** | Blocco `[HARDWARE IDENTITY]` iniettato in **8 rami** del system prompt (is_raw, non-raw, concise, greeting, web, general×2, meta) via `_hardware_identity_block()` — il modello risponde con GPU/CPU/RAM reali invece di evadere/inventare. |
