...
| `core/lifecycle.py` | Lifecycle manager: avvio componenti, shutdown graceful, RAG ingestion saltabile. Detection hardware all'avvio (`detect_hardware()` post-warmup). | `config`, `state`, `hardware` |
| `core/hardware.py` | Rilevamento identità hardware del server (GPU/VRAM/driver via `nvidia-smi`, CPU via `/proc/cpuinfo`, RAM via `/proc/meminfo`, hostname). **Solo stdlib** — testabile standalone. API: `detect_hardware()`, `get_hardware_info()`, `get_hardware_block()`. | `os`, `subprocess`, `socket`, `re` |
...
| `api/mcp/server_v2.py` | MCP v2 Streamable HTTP (24 tool + 8 resources). |
...
| `core/hardware.py` | ✅ **NUOVO** | Rilevamento hardware all'avvio (GPU via `nvidia-smi`, CPU via `/proc/cpuinfo`, RAM via `/proc/meminfo`, hostname). Solo stdlib, cache, mai eccezioni. API: `detect_hardware()`, `get_hardware_info()`, `get_hardware_block()`. |
| `agent/prompt.py` | ✅ **ESTESO** | Blocco `[HARDWARE IDENTITY]` iniettato in **7 rami** del system prompt (is_raw, non-raw, concise, greeting, web, general, meta) via `_hardware_identity_block()` — il modello risponde con GPU/CPU/RAM reali invece di evadere/inventare. |
| `api/mcp/server_v2.py` | ✅ **FIX** | 3 tool MCP con import rotti riparati: `jarvis_rag_search` (→`search_documents`), `jarvis_memory_search` (→`state.memory` via `mem0_executor`), `jarvis_web_search` (→SearXNG diretto via `state.http_client` + `SEARXNG_HOST`). Verificato live post-riavvio. |
| `openai_api/models.py` | ✅ **ESTESO** | Fase 6: `OpenAIMessage` con `content: str\|List[Dict]`, `tool_calls`, `tool_call_id`, `name` (opzionali, mai `extra='forbid'`); `ChatCompletionRequestOpenAI` con `reasoning_effort` + `stream_options`. Payload OpenAI (content array, tool_calls history) non più scartati. |
| `openai_api/chat.py` | ✅ **ESTESO** | Fase 6: flusso **`agentic` automatico** da `tools` non vuoti nel body — tool-calling emesso al client (`delta.tool_calls` + `finish_reason="tool_calls"`), **mai `execute_tool_call`** server-side; blocco `[CLIENT_TOOLS]` (~800 char, filtro `mcp__*`); `process_response_tags` saltato (header per forzarlo); `_normalize_content`; `reasoning_effort` high\|medium → thinking ON; `stream_options.include_usage` → chunk usage finale. |
| `core/chat_utils.py` | ✅ **ESTESO** | Fase 6: `build_llm_options` estrae `reasoning_effort` dal body. |
| `tests/test_agentic_contract.py` | ✅ **NUOVO** | 23 test contratto agentico E2E: round-trip tool_calls→risultato tool→testo, mai execute (spy 0 chiamate), streaming `delta.tool_calls`+`finish_reason="tool_calls"`+usage, content array normalizzati, isolamento `sys.modules` con fixture autouse. Suite totale **92/92 PASS** in unica run pytest (ordine: `agentic → hardware → prompt_hardware → mcp_tools`). |
| `tests/test_agentic_workflows.py` | ✅ **NUOVO** | 12 test workflow di sviluppo reale: feature development multi-giro (read→edit→test→finale), tool failure (errore al modello), tool paralleli, storia 5 turni, streaming frammentato ricostruito, tool_choice/errore motore/tool aziendali, isolamento conversazioni. Re-import + pop `sys.modules` → passano in entrambi gli ordini con contract (35/35). Suite totale **104/104 PASS** (ordine: `agentic_contract → agentic_workflows → hardware → prompt_hardware → mcp_tools`). |
...
|---|---|---|
| **03/08** | **Test workflow di sviluppo reale (flusso agentic) + push** | `tests/test_agentic_workflows.py` (NUOVO, 12 test): simulazioni realistiche del lavoro quotidiano con client agentico — feature development multi-giro (read→edit→test→risposta finale con history cumulativa preservata), tool failure (errore del tool arriva al modello), tool calls paralleli (più `<tool_call>` XML → tutti emessi, risultati con tool_call_id), conversazione lunga 5 turni (ruoli/tool_calls/tool_call_id intatti), streaming frammentato (arguments JSON su più chunk ricostruiti per indice), tool_choice propagato, errore motore → 500/chunk SSE, tool aziendali custom in `[CLIENT_TOOLS]` (filtro `mcp__*`), isolamento conversazioni. **Robustezza**: re-import forzato di `openai_api.chat` + pop da `sys.modules` → i due file agentic passano in entrambi gli ordini (35/35). **Test: 104/104 PASS** in unica run pytest (`agentic_contract → agentic_workflows → hardware → prompt_hardware → mcp_tools`). py_compile OK. **Pushato su origin/main.** |
| **03/08** | **Fase 6: Compatibilità client agentici (OpenCode & Co.) + push** | `jarvis/openai_api/models.py`: `OpenAIMessage` esteso (`content: str\|List[Dict]`, `tool_calls`, `tool_call_id`, `name` — opzionali, mai `extra='forbid'`) + `ChatCompletionRequestOpenAI` (`reasoning_effort`, `stream_options`). `jarvis/openai_api/chat.py`: flusso **`agentic` automatico** da `tools` non vuoti nel body (`is_agentic = bool(body.get("tools"))` — fix bug: `model_dump()` include chiavi `None`, `"tools" in body` era sempre True) — tool-calling emesso al client come `delta.tool_calls` + `finish_reason="tool_calls"`, **MAI `execute_tool_call`** server-side; blocco `[CLIENT_TOOLS]` nel system prompt (~800 char, filtro `mcp__*`); `process_response_tags` saltato (header `X-Jarvis-Process-Tags: true` per forzarlo); `_normalize_content` (content array); `_estimate_usage`; `reasoning_effort` high\|medium → thinking ON; `stream_options.include_usage` → chunk usage finale. `jarvis/core/chat_utils.py`: `build_llm_options` estrae `reasoning_effort`. **`tests/test_agentic_contract.py` (NUOVO, 23 test)**: contratto agentico E2E — round-trip tool_calls→risultato tool→testo, mai execute (spy 0 chiamate), streaming `delta.tool_calls`+`finish_reason="tool_calls"`+usage, content array normalizzati, isolamento `sys.modules` con fixture autouse (lazy import `parse_qwen_tool_calls` da `core.llm_engine` a runtime). **Test: 92/92 PASS in unica run pytest** (ordine vincolato: `agentic → hardware → prompt_hardware → mcp_tools`, quest'ultimo per ultimo perché il suo mock `rag.engine` deve vincere). py_compile OK. **Pushato su origin/main.** |
| **03/08** | **Test Suite Hardware Identity + Fix bug latente** | 3 nuovi file test (69 test, 69/69 PASS): `tests/test_hardware.py` (33 — `_run`/`_detect_gpu`/`_detect_cpu`/`_detect_ram`/`detect_hardware`/`get_hardware_info`/`get_hardware_block`/integrazione reale, mock stdlib-only), `tests/test_prompt_hardware.py` (18 — iniezione `[HARDWARE IDENTITY]` in tutti i 7 rami di `build_omniscient_prompt` + test strutturale call site = 7: la doc dichiarava 8 → **allineata a 7** in AGENTS/README/PIPELINE/COMPONENTS/CHANGELOG), `tests/test_mcp_tools.py` (18 — i 3 tool MCP riparati: `jarvis_rag_search`/`jarvis_memory_search`/`jarvis_web_search` con mock realistici). **Fix bug latente** (`agent/prompt.py`): ramo concise usava `__import__('telemetry', fromlist=['LlmCallRecord'])` → modulo inesistente, `ModuleNotFoundError` raggiungibile in produzione (`concise=True`); ora `LlmCallRecord` importato da `core.telemetry`. **Fix robustezza** (`core/hardware.py`): `get_hardware_block()` avvolto in try/except (mai eccezioni, contratto coerente). **Pushato su origin/main.** |
| **03/08** | **Hardware Identity Block + Fix MCP tool imports + push** | `core/hardware.py` (NUOVO, solo stdlib): rilevamento GPU/VRAM/driver via `nvidia-smi`, CPU via `/proc/cpuinfo`, RAM via `/proc/meminfo`, hostname. Cache + mai eccezioni. API: `detect_hardware()`/`get_hardware_info()`/`get_hardware_block()`. `core/lifecycle.py`: `detect_hardware()` all'avvio post-warmup (log `🖥️ Hardware rilevato`). `agent/prompt.py`: helper `_hardware_identity_block()` + blocco `[HARDWARE IDENTITY]` iniettato in **7 rami** del system prompt (is_raw, non-raw, concise, greeting, web, general, **meta**). **Fix ramo meta** (trace `d2811fb00043`): "Che hardware hai?" era intent `meta` e quel ramo non iniettava system prompt → modello inventava ("Apple M2 Pro Mac"); ora inietta `GENERAL_CONVERSATION_SYSTEM` + hardware block. `api/mcp/server_v2.py` (**FIX**): 3 tool MCP con import rotti — `jarvis_rag_search` (→`search_documents`), `jarvis_memory_search` (→`state.memory` via `mem0_executor`), `jarvis_web_search` (→SearXNG diretto via `state.http_client` + `SEARXNG_HOST`). **Verifica live post-riavvio** (trace `4c7b101fa70b`): risposta con RTX 3050 Ti / i5-11300H / 15.4 GiB reali, system_prompt contiene `HARDWARE IDENTITY`, prompt_tokens 175→552. Test standalone ramo meta PASS. py_compile OK. **Pushato su origin/main.** |
